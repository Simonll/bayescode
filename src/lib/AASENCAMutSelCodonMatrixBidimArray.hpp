...
#include "AASENCAMutSelOmegaCodonSubMatrix.hpp"
#include "BidimArray.hpp"

/**
 * \brief A BidimArray of AASENCAMutSelOmegaCodonSubMatrix
 *
 */

class AASENCAMutSelCodonMatrixBidimArray : public BidimArray<SubMatrix>,
                                           public BidimArray<AASENCAMutSelOmegaCodonSubMatrix> {
  public:
#
    //! constructor parameterized by an array of fitness profiles, a codon
    //! state space and a single nucleotide matrix.
    AASENCAMutSelCodonMatrixBidimArray(const CodonStateSpace *incodonstatespace,
        ...
        : codonstatespace(incodonstatespace),
          ...
          matrixarray(infitnessarray->GetSize(),
              std::vector<AASENCAMutSelOmegaCodonSubMatrix *>(
                  indelta_omega_array->GetSize(), (AASENCAMutSelOmegaCodonSubMatrix *)0)) {
        Create();
    }

    ~AASENCAMutSelCodonMatrixBidimArray() { Delete(); }
    ...
    virtual const AASENCAMutSelOmegaCodonSubMatrix &GetVal(int i, int j) const override {
        return *matrixarray[i][j];
    }
    //! non const access to the matrix for row (fitness cat) i and column (omega cat) j
    virtual AASENCAMutSelOmegaCodonSubMatrix &operator()(int i, int j) override {
        return *matrixarray[i][j];
    }
    ...
    void Create() {
        for (int i = 0; i < GetNrow(); i++) {
            for (int j = 0; j < GetNcol(); j++) {
                matrixarray[i][j] = new AASENCAMutSelOmegaCodonSubMatrix(codonstatespace, nucmatrix,
  ...
  private:
    ...
    double omega_shift;
    std::vector<std::vector<AASENCAMutSelOmegaCodonSubMatrix *>> matrixarray;
};
